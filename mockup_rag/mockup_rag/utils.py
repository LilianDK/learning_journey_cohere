...
import pandas as pd
import requests
from mockup_rag.config import COHERE_MODEL, VECTOR_NAME, client, co
...
def get_cohere_embedding(
    text: Union[str, List[str]], model_name: str = "embed-english-light-v2.0") -> List[float]:
    """
    Embed a single text with cohere client and return list of floats
    """
    if isinstance(text, str):
        embed = co.embed(texts=[text], model=model_name).embeddings[0]
    else:
        embed = co.embed(texts=text, model=model_name).embeddings
    return embed
...
def format_search_output(out: Dict) -> pd.DataFrame:
    ...
    """
    columns = ["score", "paragraph", "text", VECTOR_NAME]
    out = out["hits"]["hits"]
    data = [
        [
            o["_score"],
